...
Live countdown, real championship standings, the full calendar, driver photos &
walk-out songs, a complete **stats archive back to 1950**, and analysis charts
over the whole history of the sport.
...
- **Walk-out songs** — per-driver signature tracks with a global music toggle. Add YouTube IDs in `lib/f1/songs.ts` to enable in-app playback.
- **Track maps** — real circuit outlines on race pages (open `f1-circuits` GeoJSON).
...
- *(future)* **[OpenF1](https://openf1.org)** — live timing / telemetry.
